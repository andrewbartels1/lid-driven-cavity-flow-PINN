...
def read_datafile(filepath: str) -> tuple[np.array, np.array, np.array, np.array,
                                          int]:
    ...
    rowpts, colpts = round(np.sqrt(rows)), round(np.sqrt(rows)),
...
def get_data_files(folder_path: str = "../data/", file_ending: str = '.mat') -> list:
    ...
    folder_path = folder_path + \
        os.path.sep if (folder_path[-1] is not os.path.sep) else folder_path
...
def generate_csv_catalog(filename: str = 'catalog.csv',
    ...
    # make sure the filesep is in there for glob
    folder_path = dataDir + \
        os.path.sep if (dataDir[-1] is not os.path.sep) else dataDir

    file_list = glob.glob(
        "".join([folder_path + "**", os.sep, "**" + file_ending]))
    ...
    df = pd.DataFrame.from_records(
        data, columns=columns).sort_values(by=['Re', "filepath"])
            ...
            if (model.layers[layer_num].weight.shape[0] == 20) and \
               (model.layers[layer_num].weight.shape[1] == 20):
                ...
                all_model_weights.append(
                    model.layers[layer_num].weight.flatten())

    return torch.stack(all_model_weights).flatten()


def shape_weights_to_xygrid_flattened_size(input_tensor: torch.Tensor,
    ...
    right_tensor_len = input_tensor.unsqueeze(1).repeat(
        1, int(np.ceil(length/input_tensor.shape[0])), 1).squeeze(2)

    shaped_tensor = right_tensor_len[:, 0:length].repeat(
        desired_output_size[0], 1)
...
def navier_calc(x, y, t, lambda1, lambda2, device, Re, model,
    ...
    realtime_model_weights_of_linear_layers = all_model_weights.to(
        device)  # also known as psi in NavierStokes.py code
    ...
    psi = shape_weights_to_xygrid_flattened_size(realtime_model_weights_of_linear_layers,
                                                 y.shape,
                                                 y.shape[-1] - weights_shape[0]).to(device)
    pressure = shape_weights_to_xygrid_flattened_size(realtime_model_weights_of_linear_layers,
                                                      y.shape,
                                                      y.shape[-1] - weights_shape[0]).to(device)
    ...
    u = grad((psi * y), (psi, y),  create_graph=True,
             grad_outputs=torch.ones_like(y), allow_unused=True)[0]
    v = -grad((psi * x), (psi, x), create_graph=True,
              grad_outputs=torch.ones_like(x), allow_unused=True)[0]

    u_t = grad((u * t), (u, t), create_graph=True,
               grad_outputs=torch.ones_like(t), allow_unused=True)[0]
    u_x = grad((u * x), (u, x), create_graph=True,
               grad_outputs=torch.ones_like(x), allow_unused=True)[0]
    u_y = grad((u * y), (u, y), create_graph=True,
               grad_outputs=torch.ones_like(y), allow_unused=True)[0]
    u_xx = grad((u_x * x), (u_x, x), create_graph=True,
                grad_outputs=torch.ones_like(x), allow_unused=True)[0]
    u_yy = grad((u_y * y), (u_y, y), create_graph=True,
                grad_outputs=torch.ones_like(y), allow_unused=True)[0]

    v_t = grad((v * t), (v, t), create_graph=True,
               grad_outputs=torch.ones_like(t), allow_unused=True)[0]
    v_x = grad((v*x), (v, x), create_graph=True,
               grad_outputs=torch.ones_like(x), allow_unused=True)[0]
    v_y = grad((v*y), (v, y), create_graph=True,
               grad_outputs=torch.ones_like(y), allow_unused=True)[0]
    v_xx = grad((v_x*x), (v_x, x), create_graph=True,
                grad_outputs=torch.ones_like(x), allow_unused=True)[0]
    v_yy = grad((v_y*y), (v_y, y), create_graph=True,
                grad_outputs=torch.ones_like(y), allow_unused=True)[0]

    p_x = grad((pressure * x), (pressure, x), create_graph=True,
               grad_outputs=torch.ones_like(x), allow_unused=True)[1]
    p_y = grad((pressure * y), (pressure, y), create_graph=True,
               grad_outputs=torch.ones_like(y), allow_unused=True)[1]
