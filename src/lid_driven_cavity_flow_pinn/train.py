...
def train_model(train_dl, model, max_epochs, device, layers,
    ...
    if optimizer == "Adam":
        ...
        optimizer = LBFGS(optim_param, lr=0.01,
                          tolerance_grad=1.0 * np.finfo(float).eps)
    else:
        raise TypeError("""optimizer *must* be one of the following:\n
                           [Adam, SGD, LBFGS]""")
                           
    # super important to update the lambdas in the optimizer! 
    # the best way to do this is to store them IN the model as seen in 
    # BoxFlowNet above
    optimizer.add_param_group({'params': [model.lambda1, model.lambda2], "lr": 0.01,
                               "name": "lambdas"})
    ...
    for epoch in range(max_epochs):
        ...
        for i, sample in enumerate(train_dl):
            ...
            u_pred, v_pred, pressure_pred, f_u_pred, f_v_pred = navier_calc(
                x, y, time, model.lambda1, model.lambda2, device, Re, model,
                layers)
            ...
            if i % 100 == 0:
                ...
                # print(train_loss)
                print(f"loss: {(loss.item()/len(train_dl)):>7f}\n",
                      "time: {%.2f}" % (elapsed))
        ...
        print("saving current pytorch model to: ", os.path.join(os.getcwd(), state_dict_path))
