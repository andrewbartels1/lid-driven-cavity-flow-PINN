...
def GetStarredVelocities(space, fluid):
    ...
    u2_y = (u[2:, 1:cols+1]-2*u[1:rows+1, 1:cols+1] +
            u[0:rows, 1:cols+1])/(dy**2)
    u2_x = (u[1:rows+1, 2:]-2*u[1:rows+1, 1:cols+1] +
            u[1:rows+1, 0:cols])/(dx**2)
    v_face = (v[1:rows+1, 1:cols+1]+v[1:rows+1, 0:cols] +
              v[2:, 1:cols+1]+v[2:, 0:cols])/4
    u_star[1:rows+1, 1:cols+1] = u[1:rows+1, 1:cols+1]-dt * \
        (u[1:rows+1, 1:cols+1]*u1_x+v_face*u1_y) + \
        (dt*(mu/rho)*(u2_x+u2_y))+(dt*S_x)

    v1_y = (v[2:, 1:cols+1]-v[0:rows, 1:cols+1])/(2*dy)
    v1_x = (v[1:rows+1, 2:]-v[1:rows+1, 0:cols])/(2*dx)
    v2_y = (v[2:, 1:cols+1]-2*v[1:rows+1, 1:cols+1] +
            v[0:rows, 1:cols+1])/(dy**2)
    v2_x = (v[1:rows+1, 2:]-2*v[1:rows+1, 1:cols+1] +
            v[1:rows+1, 0:cols])/(dx**2)
    u_face = (u[1:rows+1, 1:cols+1]+u[1:rows+1, 2:] +
              u[0:rows, 1:cols+1]+u[0:rows, 2:])/4
    v_star[1:rows+1, 1:cols+1] = v[1:rows+1, 1:cols+1]-dt * \
        (u_face*v1_x+v[1:rows+1, 1:cols+1]*v1_y) + \
...
def SolvePressurePoisson(space, fluid, left, right, top, bottom):
    ...
    while(error > tol):
        ...
        p2_xy = (p_old[2:, 1:cols+1]+p_old[0:rows, 1:cols+1]) / \
            dy**2+(p_old[1:rows+1, 2:]+p_old[1:rows+1, 0:cols])/dx**2
        p[1:rows+1, 1:cols+1] = (p2_xy)*factor - \
            (rho*factor/dt)*(ustar1_x+vstar1_y)
...
def WriteToFile(dir_path, space, iteration, interval, Re, timestep):
    if(iteration % interval == 0):
        # appended Re for ML applications
        # dir_path = os.path.join(os.getcwd(), "Results", "Re{}".format(int(Re)))
        dir_path = os.path.join(dir_path, "Re{}".format(int(Re)))
        ...
        with open(path, "w") as f:
            for i in range(space.rowpts):
                for j in range(space.colpts):
                    f.write("{}\t{}\t{}\t{}\n".format(
                        space.p_c[i, j], space.u_c[i, j], space.v_c[i, j], timestep))
